            ...
            <span class="project_pm_box">
                ...
                <span class="pm_list_box">
                    ...
                    <span class="list_content">
                        ...
                        <span class="lc_period"><?=$psInfos[$i]['pjts_startdate']?> ~ <?=$psInfos[$i]['pjts_duedate']?><span class="period_status period_status_color_g">(<?=$psInfos[$i]['residual_day']?> / <?=$psInfos[$i]['pjts_status']?>)</span></span>
                        ...
                        <span class="lc_work_list">
                            ...
                            <span class="lcw_work_list"><?=$pdInfos[$x]['pjtd_name']?><span class="wl_date"><?=$pdInfos[$x]['pjtd_startdate']?> ~ <?=$pdInfos[$x]['pjtd_duedate']?><span class="period_status period_status_color_g">(<?=$pdInfos[$x]['residual_day']?> / <?=$pdInfos[$x]['auto_status']?>)</span></span></span>
						...
						<span class="lc_work_list_add" id="pdModify_<?=$pdInfos[$x]['pjtd_no']?>" style="display:none">
                            <form id="projectDetailModifyForm_<?=$pdInfos[$x]['pjtd_no']?>" name="projectDetailModifyForm_<?=$pdInfos[$x]['pjtd_no']?>" method="POST">
                                <b>작업 항목 수정</b>
                                <span class="lc_work_list_add_text"><input type="text" id="pdName" name="pdName"></span>
                                <span class="lc_work_list_add_text"><input type="text" id="pdComment" name="pdComment" placeholder="수정 사유를 입력하세요."></span>
                                <span class="lc_work_list_add_date">
                                    <input type="date" id="pdStartdate" name="pdStartdate">
                                    <input type="date" id="pdDuedate" name="pdDuedate">
                                </span>
    ...
    <ul class="schedule_box memo_hide_button view_none">
    	...
    	<li class="sb_project">
            ...
            <span class="section_schedule">
				...
				<textarea id="pMemo" placeholder="프로젝트와 관련된 메모를 입력해주세요."></textarea>
            </span>
        </li>
		<!--메모-->
	...
	<ul class="schedule_box2">
    	...
    	<li class="sb_project">
			...
			<span class="calender_info_list_box">
				...
				<span class="calender_info_list"><span><?=$schedule[$i]['pjts_startdate']?> ~ <?=$schedule[$i]['pjts_duedate']?><b class="period_status_color_g">(<?=$schedule[$i]['residual_day']?> / <?=$schedule[$i]['auto_status']?>)</b></span><span>/</span><span><?=$schedule[$i]['pjt_name']?></span><span>/</span><span><?=$schedule[$i]['pjts_name']?></span></span>
                <?php } ?>
            </span>
        </li>
        <!--// Project Work List -->
    	...
    	<li class="sb_project">
			...
			<span class="calender_info_list_bar"></span> -->
			<span class="calender_info_list_box">
				<span class="calender_info_list"><span>2023.03.10</span><span>/</span><span>휴가</span><span>/</span><span>홀길동 휴가</span></span>
				<span class="calender_info_list"><span>2023.03.15</span><span>/</span><span>행사</span><span>/</span><span>전체 회식</span></span>
			</span>
        </li>
        <!--// Event List -->
...
<script>
    ...
    function modifyPd(ths) { //작업항목 수정
        ...
        $.ajax({
            ...
            async: true,
            success: function(rslt) {
                $('input[id=pdName]').val(rslt.pjtd_name);
                $('input[id=pdStartdate]').val(rslt.pjtd_startdate);
                $('input[id=pdDuedate]').val(rslt.pjtd_duedate);
